...
def _create_schema(conn: sqlite3.Connection) -> None:
    ...
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS historic_players (
            ...
            matches_played_for_vasco INTEGER,
            FOREIGN KEY (player_id) REFERENCES players (id) ON DELETE CASCADE
        );
    ...
    if "matches_played_for_vasco" not in historic_cols:
        conn.execute("ALTER TABLE historic_players ADD COLUMN matches_played_for_vasco INTEGER")
...
def _parse_goal_minutes(item: Any) -> list[int]:
...
def _parse_goal_periods(item: Any) -> list[str]:
    if not isinstance(item, list):
        return []
    out: list[str] = []
    for periodo in item:
        txt = str(periodo or "").strip()
        if txt:
            out.append(txt)
    return out
...
def _parse_goal_item(item: Any) -> tuple[str | None, int, str | None, list[int], list[str]]:
    if isinstance(item, dict):
        ...
        try:
            ...
            goals = 1
        minutes = _parse_goal_minutes(item.get("minutos", []))
        periods = _parse_goal_periods(item.get("periodos", []))
        if not minutes:
            minuto_unico = item.get("minuto")
            try:
                minuto_val = int(minuto_unico)
            except Exception:
                minuto_val = None
            if minuto_val is not None and 0 <= minuto_val <= 120:
                minutes = [minuto_val]
        if not periods:
...
def _parse_card_item(item: Any) -> tuple[str | None, int, str | None]:
    if isinstance(item, dict):
        ...
        club = str(item.get("clube", "")).strip() or None
        try:
            count = int(item.get("cartoes", item.get("qtd", 1)))
        except Exception:
            count = 1
    else:
        name = str(item or "").strip()
...
def save_matches(db_path: str, jogos: list[dict[str, Any]]) -> None:
    ...
    with _open(db_path) as conn:
        ...
        conn.execute("DELETE FROM match_cards")
        ...
        for jogo in jogos:
            ...
            for item in jogo.get("cartoes_amarelos_vasco", []) if isinstance(jogo.get("cartoes_amarelos_vasco"), list) else []:
                name, count, club = _parse_card_item(item)
                if not name:
                    continue
                player_id = _ensure_player(conn, name)
                conn.execute(
                    ...
                    VALUES (?, 'vasco', ?, ?, 'amarelo', ?, ?)
                    """,
                    (match_id, player_id, name, count, club),
                )

            for item in jogo.get("cartoes_vermelhos_vasco", []) if isinstance(jogo.get("cartoes_vermelhos_vasco"), list) else []:
                name, count, club = _parse_card_item(item)
                if not name:
                    continue
                player_id = _ensure_player(conn, name)
...
def load_matches(db_path: str) -> list[dict[str, Any]]:
    with _open(db_path) as conn:
        ...
        for g in goals_rows:
            ...
            try:
                minutes = json.loads(g["goal_minutes_json"] or "[]")
            except Exception:
                minutes = []
            minutes = _parse_goal_minutes(minutes)
            try:
                periods = json.loads(g["goal_periods_json"] or "[]")
            except Exception:
                periods = []
        ...
        cards_by_match: dict[int, dict[str, list[dict[str, Any]]]] = {}
        cards_rows = conn.execute(
            """
            SELECT match_id, side, player_name, card_type, card_count, club_name
            FROM match_cards
            ORDER BY id
            """
        ).fetchall()
        for c in cards_rows:
            mid = int(c["match_id"])
            bucket = cards_by_match.setdefault(
                mid,
                {
            ...
            payload = {"nome": c["player_name"], "cartoes": int(c["card_count"])}
            if c["club_name"]:
                payload["clube"] = c["club_name"]
            if c["side"] == "vasco" and c["card_type"] == "amarelo":
                bucket["cartoes_amarelos_vasco"].append(payload)
            elif c["side"] == "vasco" and c["card_type"] == "vermelho":
                bucket["cartoes_vermelhos_vasco"].append(payload)
        ...
        jogos.append(
            {
                ...
                "cartoes_amarelos_vasco": cards.get("cartoes_amarelos_vasco", []),
    ...
    with _open(db_path) as conn:
        ...
        for item in jogadores:
            if isinstance(item, dict):
                ...
                matches_played = item.get("jogos_pelo_vasco")
            else:
                nome = str(item or "").strip()
                posicao = ""
                registered_date = ""
            ...
            conn.execute(
                ...
                INSERT OR REPLACE INTO historic_players(
                    ...
                    matches_played_for_vasco
                ) VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (pid, posicao, registered_date, joined_date, left_date, passages_json, matches_played),
            )
...
def load_historic_players(db_path: str) -> dict[str, Any]:
    with _open(db_path) as conn:
        ...
        rows = conn.execute(
            """
            SELECT p.name, h.position, h.registered_date_text, h.joined_date_text, h.left_date_text,
                   h.passages_json, h.matches_played_for_vasco
        ...
        "jogadores": [
            {
                ...
                "jogos_pelo_vasco": (
                    int(row["matches_played_for_vasco"])
                    if row["matches_played_for_vasco"] is not None
                    else None
                ),
            }
            for row in rows
